...
public class PartitionDedupMetadataManagerTest {
  ...
  public void verifyAddRemoveSegment() {
    ...
    Cache<Object, IndexSegment> recordLocationMap = metadataManager._primaryKeyToSegmentMap;
...
//    metadataManager.addSegment(segment1);
    ...
    ImmutableSegmentImpl segment1 = mockSegment(1);
    Assert.assertFalse(metadataManager.checkRecordPresentOrUpdate(getPrimaryKey(0), segment1));
    ...
    Assert.assertTrue(metadataManager.checkRecordPresentOrUpdate(getPrimaryKey(3), segment1));
    checkRecordLocation(recordLocationMap, 3, segment2, hashFunction);

    Thread.sleep(1000);
    Assert.assertFalse(metadataManager.checkRecordPresentOrUpdate(getPrimaryKey(3), segment1));
    checkRecordLocation(recordLocationMap, 3, segment1, hashFunction);
...
//    checkRecordLocation(recordLocationMap, 3, segment1, hashFunction);
  ...
  private static void checkRecordLocation(Cache<Object, IndexSegment> recordLocationMap, int keyValue,
      IndexSegment segment, HashFunction hashFunction) {
    IndexSegment indexSegment =
        recordLocationMap.getIfPresent(HashUtils.hashPrimaryKey(getPrimaryKey(keyValue), hashFunction));
